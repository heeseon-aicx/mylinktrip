...
Deno.serve(async (req) => {
  ...
  try {
    ...
    if (claimError || !claimed) {
      console.log(
        `[process_link] Link ${linkId} already processing or not found`
      );
      return new Response(
        JSON.stringify({
          ok: false,
    ...
    console.log(
      `[process_link] Analyzing video with Gemini (${ytMeta.durationSeconds}s, ${totalChunks} chunks)...`
    );
    await updateProgress(
      supabase,
      linkId,
      30,
      "extract_places",
      `영상 속 장소를 추출 중이에요 (${totalChunks}개 구간)`
    );

    const geminiResult = await analyzeVideoWithGemini(
      youtubeUrl,
      ytMeta.durationSeconds
    );
    ...
    console.log(
      `[process_link] Gemini result: plan_title=${
        geminiResult.plan_title
      }, places=${geminiResult.places?.length || 0}`
    );
    await updateProgress(
      supabase,
      linkId,
    ...
    await completeLink(
      supabase,
      linkId,
      geminiResult.plan_title || ytMeta.title
    );
